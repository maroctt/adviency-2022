<template>
    ...
    <input type="text" placeholder="Ingrese el URL de la imagen" v-model="regalo.imgUrl"  class="form-control inline-block w-full mt-2 px-4 py-2 text-sm font-normal text-gray-800 bg-c-beige bg-clip-padding border-2 border-solid border-c-black rounded-full transitions ease-in-out m-0 focus:border-c-green focus:outline-none focus:bg-c-beige-100 ">
        ...
        agregarItem(){
            if(this.regalo.nombre != ''){
                ...
                if(this.checkRegalo(value, this.listaRegalos)){
                    ...
                    this.listaRegalos.push({nombre: value, cantidad:this.regalo.cantidad, imgUrl:this.regalo.imgUrl});
                    this.limpiarInputs()
                }
            }else{
